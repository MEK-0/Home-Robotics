...
## 4. Robot and Linear Rail Configuration
...
With the project world convention:
...
the nominal rail axis is:

```text
±X
```

Each robot therefore has the following manipulation chain:
...
For motion planning, the rail is part of the robot kinematic system.

It is not a manual teleport mechanism.
...
## Linear Rail Planning Rule

The rail joint must be represented consistently in:
...
A normal reach task may be solved by:

```text
rail motion only
arm motion only
or coordinated rail + arm motion
```

The system must not hard-code a separate rail position for every object unless that value belongs to an explicitly documented semantic waypoint or validated configuration.

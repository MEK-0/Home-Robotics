...
int main(int argc,char** argv) {
  ...
  std::thread spinner([&](){executor.spin();});int result=1; S stage=S::IDLE;
  auto transition=[&](S next,const char* name){require(hm::transition_allowed(stage,next),"Invalid manipulation transition");stage=next;RCLCPP_INFO(log,"STATE -> %s",name);};
  bool execute=false,stabilized=false,attached=false,paused=false;
  try {
    std::string robot,object;double height;
    ...
    require(robot=="panda1"&&object=="cube","Only Panda1/cube supported");require(hm::validLiftHeight(height),"lift_height must be finite and 0.08..0.15 m");
    ...
    auto contact=[&](){std_msgs::msg::String message;
      require(rclcpp::wait_for_message(message,reader,"/mujoco/objects/cube/contact",2s),"NO_CONTACT missing contact interface");
      auto c=YAML::Load(message.data);double age=node->now().seconds()-c["timestamp"].as<double>();
      require(age>=-0.1&&age<=0.5,"OBJECT_DROPPED stale physics state");require(c["object_id"].as<std::string>()=="cube","Wrong contact object");return c;};
    auto pose=[&](){geometry_msgs::msg::PoseStamped p;
      require(rclcpp::wait_for_message(p,reader,"/mujoco/objects/cube/pose",2s),"OBJECT_NOT_FOUND cube");
    ...
    hm::ObjectRegistry registry(config+"objects.yaml");auto initial_pose=pose();print_pose(log,"Initial cube",initial_pose.pose);
    ...
    const auto support_id=YAML::LoadFile(config+"objects.yaml")["objects"]["cube"]["initial"]["support_surface"].as<std::string>();
    auto checked=[&](planning_scene::PlanningScene& sc,moveit::core::RobotState& st,
      ...
      if(response.collision){
        ...
        const auto* body=st.getAttachedBody("cube");
        ...
        for(const auto& [pair,contacts]:response.contacts){
          bool support=support_region&&((pair.first=="cube"&&pair.second==support_id)||(pair.second=="cube"&&pair.first==support_id));
    ...
    auto gate=[&](){
      ...
      auto p=pose();auto cube=scene.getWorld()->getObject("cube");
      ...
      for(const auto& link:group.getRobotModel()->getLinkModelNames()){
        ...
        require(!scene.getAllowedCollisionMatrix().getAllowedCollision(link,"cube",type)||type==collision_detection::AllowedCollision::NEVER,"Unexpected global cube allowance");}
    ...
    hm::GraspParameters parameters;parameters.finger_open_width=registry.at("cube").geometry.primitives[0].dimensions[0]+0.014;
    auto candidate=hm::generateTopDownGrasp(registry.at("cube"),initial_pose.pose,hm::pandaGripperGeometry(group.getRobotModel()),parameters);
    ...
    auto cartesian=[&](const moveit::core::RobotState& start,const geometry_msgs::msg::Pose& target,const collision_detection::AllowedCollisionMatrix& acm){
      ...
      trajectory_processing::TimeOptimalTrajectoryGeneration timing(0.000001, 0.02, 0.0001);require(timing.computeTimeStamps(tr,0.04,0.04),"Retiming failed");
    ...
    auto run=[&](const Plan& p,const collision_detection::AllowedCollisionMatrix& acm,const char* label){
      ...
      while(std::chrono::steady_clock::now()<deadline){
        ...
        if(stabilized){auto c=contact();if(!c["stabilization_active"].as<bool>()||c["relative_drift"].as<double>()>0.01||c["unexpected_contacts"].size()>0){group.stop();throw std::runtime_error("OBJECT_DROPPED or unexpected cube collision");}}
    ...
    if(!execute){
      ...
      a.object=hm::world_object(registry.at("cube"),initial_pose);a.object.header.frame_id="panda1_hand";
      ...
      cartesian(lifted,candidate.grasp_pose,original_acm);
      ...
      trigger("/mujoco/cube/begin_grasp");transition(S::GRIPPER_CLOSING,"GRIPPER_CLOSING");gripper.close();
      ...
      try{trigger("/mujoco/cube/verify_grasp");}catch(...){
      ...
      transition(S::GRASP_VERIFIED,"GRASP_VERIFIED");
      RCLCPP_INFO(log,"Stabilize %s",bool_service("/mujoco/cube/stabilize",true).c_str());stabilized=true;
      // Observe a short simulation-time settling interval and enforce activation displacement.
      auto c=contact();double t=c["timestamp"].as<double>();do{c=contact();require(c["activation_jump"].as<double>()<=0.005,"GRASP_UNSTABLE activation jump");}while(c["timestamp"].as<double>()-t<0.12);
      ...
      moveit_msgs::msg::CollisionObject remove;remove.id="cube";remove.operation=remove.REMOVE;diff.world.collision_objects.push_back(remove);
      moveit_msgs::msg::AttachedCollisionObject a;a.link_name="panda1_hand";a.touch_links=candidate.allowed_touch_links;
      a.object=hm::world_object(registry.at("cube"),current_cube);a.object.header.frame_id="panda1_hand";
      ...
      auto attached_scene=get_scene();int count=0;for(const auto& x:attached_scene.robot_state.attached_collision_objects)if(x.object.id=="cube")++count;
      for(const auto& x:attached_scene.world.collision_objects)require(x.id!="cube","Duplicate WORLD cube");require(count==1,"Attachment not unique");attached=true;pause(false);RCLCPP_INFO(log,"WORLD -> ATTACHED verified: world_cube=0 attached_cube=1");
      ...
      do{c=contact();require(c["stabilization_active"].as<bool>()&&c["relative_drift"].as<double>()<=0.01,"OBJECT_DROPPED hold");}while(c["timestamp"].as<double>()-t<1.5);
      transition(S::PREPLACE_PLANNED,"PREPLACE_PLANNED");transition(S::PLACING,"PLACING");
      scene.setPlanningSceneMsg(get_scene());fresh_state(state,reader,node);
      auto descent=cartesian(state,toPose(grasp_tcp),original_acm);run(descent,original_acm,"Return descent");
      c=contact();require(c["support_contact"].as<bool>(),"PLACE_FAILED no physical table support; holding grasp");
      RCLCPP_INFO(log,"Return support contact=true");
      transition(S::RELEASING,"RELEASING");bool_service("/mujoco/cube/stabilize",false);stabilized=false;RCLCPP_INFO(log,"Stabilization removed after support verification");
      pause(true);diff=moveit_msgs::msg::PlanningScene();diff.is_diff=true;diff.robot_state.is_diff=true;
      a=moveit_msgs::msg::AttachedCollisionObject();a.link_name="panda1_hand";a.object.id="cube";a.object.operation=a.object.REMOVE;
      diff=hm::releaseCube(hm::world_object(registry.at("cube"),pose()),c["support_contact"].as<bool>(),stabilized);apply(diff);
      auto detached=get_scene();count=0;for(const auto& x:detached.world.collision_objects)if(x.id=="cube")++count;
      for(const auto& x:detached.robot_state.attached_collision_objects)require(x.object.id!="cube","Stale attached cube");require(count==1,"WORLD cube not unique");attached=false;pause(false);RCLCPP_INFO(log,"ATTACHED -> WORLD verified: world_cube=1 attached_cube=0");
      ...
      auto retreat=cartesian(state,candidate.pre_grasp_pose,touch);run(retreat,touch,"Retreat");
      transition(S::PLACE_VERIFYING,"PLACE_VERIFYING");auto final_pose=pose();print_pose(log,"Final cube",final_pose.pose);
      double pe=(transform(final_pose.pose).translation()-transform(initial_pose.pose).translation()).norm();
      double oe=Eigen::Quaterniond(transform(final_pose.pose).linear()).angularDistance(Eigen::Quaterniond(transform(initial_pose.pose).linear()));
      c=contact();require(pe<=0.03&&c["support_contact"].as<bool>()&&!c["stabilization_active"].as<bool>(),"PLACE_FAILED final pose/support");
      require(panda2_drift->load()<=0.002,"Panda2 drift");
      RCLCPP_INFO(log,"FINAL position_error=%.12f orientation_error=%.12f Panda2_max_drift=%.12f",pe,oe,panda2_drift->load());
      transition(S::PLACE_VERIFIED,"PLACE_VERIFIED");transition(S::DONE,"DONE");RCLCPP_INFO(log,"CUBE PICK LIFT RETURN PASS");result=0;
    }

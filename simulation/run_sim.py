#!/usr/bin/env python3
"""Headless Phase 1 scene startup and reset smoke test."""
...
from simulation.validation import validate_home_state, validate_simulation, validate_state
...
def main() -> int:
    ...
    try:
        ...
        with SceneBuilder(config).build(headless=True) as simulator:
            ...
            log.info("Physics ............ OK")
            log.info("Scene validation ... OK")
            simulator.step(5)
            validate_state(simulator)
            ResetManager(simulator, config=config, settle_steps=config.physics["reset"]["settle_steps"]).reset()
            validate_simulation(config, simulator)
            validate_home_state(config, simulator)
            log.info("Reset .............. OK")
        return 0
